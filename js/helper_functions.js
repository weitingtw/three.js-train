...
function computeNormals(cur_curveSegments) {
    ...
    for (var i = 0; i < CPlist.length; i++) {
        ...
        for (var j = 0; j < cur_curveSegments; j++) {
            ...
            normal.lerpVectors(CPlist[a].orient, CPlist[b].orient, j * (1 / cur_curveSegments));
...
function computerTimeIncrements() {
    ...
    for (var i = 0; i < curves.length; i++) {
        if (params.ArcLength) {
            time_increments.push(Math.round(params.Speed / curves[i].getLength() * 200));
        } else {
...
function addPoint(world) {
    var newpointx = (CPlist[0].position().x + CPlist[CPlist.length - 1].position().x) / 2;
    var newpointy = (CPlist[0].position().y + CPlist[CPlist.length - 1].position().y) / 2;
    var newpointz = (CPlist[0].position().z + CPlist[CPlist.length - 1].position().z) / 2;
...
function drawCurves(cur_curveSegments) {
    ...
    for (var i = 0; i < CPlist.length; i++) {
        ...
        var controlpoint = new THREE.Vector3(CPlist[a].position().x + CPlist[b].position().x, CPlist[a].position().y + CPlist[b].position().y, CPlist[a].position().z + CPlist[b].position().z);
        var curve;;
        curve = new LinearCurve(CPlist[a], CPlist[b]);
        if (params.Bspline) {
            curve = new BsplineCurve(CPlist[a], CPlist[b], CPlist[c], CPlist[d]);
        }
        var geometry = new THREE.TubeGeometry(curve, cur_curveSegments, 0.5, 4, false);
...
function setupUI() {
    ...
    // view Folder
    f1.add(params, 'ArcBallView').onChange(function (value) {
        if (value == true) {
    ...
    f1.add(params, 'TopView').onChange(function (value) {
        if (value == true) {
    ...
    f1.add(params, 'TrainView').onChange(function (value) {
        if (value == true) {
            params.TopView = false;
            params.ArcBallView = false;
        }
    ...
    // html element
    document.getElementById("updateshaderbtn").addEventListener("click", function () {
        train.updateShader();
    });
}
        ...
        fileSelected.addEventListener('change', function (e) {
            ...
            fileReader.onload = function (e) {
                ...
                for (var i = 0; i < result.length / 6; i++) {
                    var new_point = new ControlPoint(parseInt(result[i * 6]), parseInt(result[i * 6 + 1]), parseInt(result[i * 6 + 2]), world);
                    new_point.setOrient(parseInt(result[i * 6 + 3]), parseInt(result[i * 6 + 4]), parseInt(result[i * 6 + 5]));
